...
export function printReceipt(booking: Booking): void {
  ...
  const location = typeof booking.workLocation === 'object'
    ? [booking.workLocation.address, booking.workLocation.city].filter(Boolean).join(', ')
    : String(booking.workLocation || '—');
...
<style>
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .print-btn{padding:10px 28px;background:#FF8C00;color:#fff;font-weight:700;font-size:13px;border:none;border-radius:8px;cursor:pointer;margin:0 6px;}
...
<script>
  // Auto-focus window for CMD+P / Ctrl+P
  window.focus();
</script>
</body>
</html>`;
