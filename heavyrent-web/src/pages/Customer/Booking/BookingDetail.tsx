      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          <div style={{
            ...
            background: booking.status === 'rejected' ? '#FFEBEE' : booking.status === 'completed' ? '#E8F5E9' : booking.status === 'in_progress' ? '#EFF6FF' : '#FFFBEB',
            border: `1px solid ${booking.status === 'rejected' ? '#FFCDD2' : booking.status === 'completed' ? '#C8E6C9' : booking.status === 'in_progress' ? '#BFDBFE' : '#FDE68A'}`,
            display: 'flex', gap: 14, alignItems: 'center',
          }}>
            <div style={{ fontSize: 36 }}>
              {booking.status === 'pending' ? '⏳' : booking.status === 'accepted' ? '✅' : booking.status === 'in_progress' ? '🔧' : booking.status === 'completed' ? '🎉' : '❌'}
            </div>
            <div style={{ flex: 1 }}>
