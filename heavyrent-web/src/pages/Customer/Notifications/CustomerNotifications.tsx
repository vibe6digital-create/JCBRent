...
const TYPE_CONFIG: Record<string, { bg: string; color: string; icon: string }> = {
  booking_approved: { bg: '#E8F5E9', color: '#43A047', icon: '✅' },
  booking_rejected: { bg: '#FFEBEE', color: '#E53935', icon: '❌' },
  booking_completed: { bg: '#EFF6FF', color: '#3B82F6', icon: '🎉' },
  booking_request: { bg: '#FFF3E0', color: '#FF8C00', icon: '📋' },
  dispatched: { bg: '#F3E8FF', color: '#9333EA', icon: '🚛' },
  general: { bg: '#F5F5F5', color: '#6B7280', icon: '🔔' },
};
    ...
    <div style={{ maxWidth: 680, margin: '0 auto' }} className="fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
        <div>
          ...
          {unreadCount > 0 && <p style={{ color: '#FF8C00', fontSize: 13, fontWeight: 600, marginTop: 4 }}>{unreadCount} unread</p>}
        </div>
        {unreadCount > 0 && (
          <button onClick={markAllRead} style={{
            display: 'flex', alignItems: 'center', gap: 6,
            padding: '8px 14px', borderRadius: 8, background: '#F5F5F5', border: '1px solid #E5E7EB',
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {notifications.map(n => {
          ...
          return (
            ...
            >
              <div style={{
                width: 42, height: 42, borderRadius: 10, background: cfg.bg,
