  ...
  if (saved) return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '60vh', gap: 16, padding: 40 }} className="fade-in">
      ...
      <div style={{ display: 'flex', gap: 12 }}>
        ...
        <button onClick={() => { setSaved(false); setForm({ category: '', model: '', description: '', hourlyRate: '', dailyRate: '', weeklyRate: '', monthlyRate: '', city: '', state: '', serviceAreas: '', isAvailable: true }); }} style={{ padding: '12px 28px', borderRadius: 10, background: '#fff', color: '#6B7280', fontWeight: 700, fontSize: 14, border: '1px solid #E5E7EB', cursor: 'pointer' }}>
          Add Another
        </button>
      </div>
    </div>
    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', border: `2px solid ${errors[field] ? '#E53935' : '#E5E7EB'}`, borderRadius: 8, overflow: 'hidden', background: '#fff' }}>
        ...
        <input type={type} value={(form as Record<string, unknown>)[field] as string} onChange={e => set(field, e.target.value)} placeholder={placeholder}
          style={{ flex: 1, padding: '10px 14px', border: 'none', fontSize: 14, color: '#1A1D26' }} />
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E5E7EB', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div>
              ...
              <textarea value={form.description} onChange={e => set('description', e.target.value)} placeholder="Describe the machine condition, features, and any special capabilities..."
                rows={3} style={{ width: '100%', padding: '10px 14px', border: '2px solid #E5E7EB', borderRadius: 8, fontSize: 14, color: '#1A1D26', resize: 'vertical' }} />
            </div>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E5E7EB', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              ...
              <Field label="Monthly Rate (₹)" field="monthlyRate" type="number" placeholder="1,60,000" prefix="₹" />
            </div>
            <p style={{ fontSize: 12, color: '#9CA3AF', marginTop: -6 }}>Weekly & monthly rates are optional. If not set, customers will see calculated rates.</p>
          </div>
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E5E7EB', padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <Field label="City *" field="city" placeholder="Pune" />
          ...
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #E5E7EB', padding: '18px 24px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => set('isAvailable', !form.isAvailable)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: form.isAvailable ? '#FF8C00' : '#9CA3AF', display: 'flex', transition: 'color 0.15s' }}>
              {form.isAvailable
          ...
          <div style={{ background: '#1A1A2E', borderRadius: 14, padding: '20px', marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
              <div style={{ width: 44, height: 44, background: 'rgba(255,140,0,0.15)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 22 }}>
                {form.category ? { JCB: '🚜', Excavator: '⛏️', Crane: '🏗️', Bulldozer: '🚧', Roller: '🛞', Pokelane: '🛣️' }[form.category] : '❓'}
              </div>
              <div>
